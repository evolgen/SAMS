...
triplets_thrice_filt1 = list(filter(lambda triplet1: triplet1[0] != triplet1[1], triplets_thrice))
triplets_thrice_list1 = []
for curr_elem_lst1 in range(0, len(triplets_thrice_filt1)):
    now_value1 = str('__'.join(map(str, triplets_thrice_filt1[curr_elem_lst1])))
            ...
            if all(nucl1 in accepted_nucleotides for nucl1 in curr_species1_nucl1): # Checking for ATGCs alone
                if all(nucl1 in accepted_nucleotides for nucl1 in curr_species2_nucl1):
                    if all(nucl1 in accepted_nucleotides for nucl1 in curr_mrca1_nucl1):
                        if(curr_species2_nucl1 != curr_species1_nucl1):
